...
async function Home() {
  ...
  const { otp } = await GET_OTP_FOR_TELEGRAM(user!.email!);
  ...
  if (monthStats) {
    ...
    data.forEach((schedule) => {
      if (toBeMapped[schedule.date]) {
        toBeMapped[schedule.date].items = [...toBeMapped[schedule.date].items, ...schedule.items];
      } else {
      ...
      <div className="overflow-hidden grid md:grid-cols-[auto_1fr] gap-5">
        ...
        <div>
          <div className="pt-5 flex justify-between">
            ...
            <Drawer avatar={profile?.avatar} otp={otp} />
          </div>
          <HomeNavButtons />
          <div className="mt-10 w-full ">
            <h1 className="text-sm xxs:text-xl xs:text-2xl">{format(new Date(), 'MMMM, y')}</h1>
