...
export default function CompleteProfile({ user, profile }: Props) {
  ...
  const onSubmit: SubmitHandler<CompleteProfileSchemaType> = async (data) => {
    startTransition(async () => {
      ...
      const { error, id } = await completeProfileAction(user.id, data, profile?.id);
      if (error || !id) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { data, error } = await supabase.storage
      .from('bible-reader')
      .upload(`temp-avatar/${file.name}${new Date().getTime().toString()}`, file, {
        cacheControl: '3600',
        upsert: false,
      });
  ...
  const hasCompletedProfile =
    profile?.first_name && profile.last_name && profile.gender && profile.avatar;
        ...
        <CardContent>
          ...
          >
            ...
            <div className="w-full">
              ...
              <Input id="lastName" type="text" {...register('lastName')} defaultValue={undefined} />
              {errors.lastName && (
            ...
            {!(user?.user_metadata?.avatar_url || user?.user_metadata.picture) && (
              <div className="w-full">
                ...
                />
                {errors.avatar && (
                  <span className="text-red-400 text-xs">{errors.avatar.message}</span>
                )}
              </div>
            )}
            {image.length ? (
